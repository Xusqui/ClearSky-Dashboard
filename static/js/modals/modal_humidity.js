...
function loadHumChart(startDate, endDate) {
    ...
    fetch(fetchUrl)
        ...
        .then((data) => {
            ...
            // casos llevan el real de cada día/hora; si no vienen (rango
            // corto, sin agregar), cada fila ya es un dato real y se usa el
            // propio campo.
            ...
            var extHum = { min: extremoPeriodo("humedad", "min"), max: extremoPeriodo("humedad", "max") };
            ...
            var humedad = data.map((row) => redondear1(parseFloat(row.humedad)));

            // Banda sombreada mín-máx real: así el "Máx"/"Mín" del periodo
            // coincide con el borde de la banda dibujada, en vez de quedar
            // como una etiqueta flotando por encima de una línea que nunca
            // llega a ese valor. Truco de ECharts: dos series apiladas
            // (stack), la primera invisible hasta el mínimo, la segunda
            // visible con la diferencia hasta el máximo -> el área visible
            // va exactamente de mín a máx.
            ...
            var humMinArr = data.map((row) => extremoFila(row, "humedad", "min"));
            var humMaxArr = data.map((row) => extremoFila(row, "humedad", "max"));

            // Escala Y dinámica basada en los extremos reales del periodo,
            // pero acotada a 0-100%
            var minY = extHum.min.valor - 5;
            var maxY = extHum.max.valor + 5;
            if (minY < 0) minY = 0;
            if (maxY > 100) maxY = 100;

            var maxIdx = data.findIndex((row) => extremoFila(row, "humedad", "max") === extHum.max.valor);
            var minIdx = data.findIndex((row) => extremoFila(row, "humedad", "min") === extHum.min.valor);
            ...
            var option = {
                ...
                series: [
                    ...serieBanda(humMinArr, humMaxArr, blueColor, "bandaHum"),
                    {
                        name: "Humedad",
                        data: humedad,
                        type: "line",
