...
function updateTempWidget() {
    fetch('./static/modules/widgets/get_temp_data.php')
        ...
        .then(data => {
            ...
            // Mostrar la fórmula utilizada debajo de la sensación térmica
            const formulaEl = document.getElementById('temp-widget-formula');
            if (formulaEl) {
                let formulaTxt = '';
                switch (data.formula) {
                    case 'wind_chill':
                        formulaTxt = '(Wind Chill)';
                        break;
                    case 'heat_index':
                        formulaTxt = '(Heat Index)';
                        break;
                    case 'steadman_vpd':
                        formulaTxt = '(Steadman VPD)';
                        break;
                    case 'steadman':
                        formulaTxt = '(Steadman)';
                        break;
                    default:
                        formulaTxt = '(Fórmula: Desconocida)';
                }
