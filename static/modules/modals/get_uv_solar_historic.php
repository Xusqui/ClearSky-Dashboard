        ...
        if ($diff_days > 30) {
            ...
            $query = "
                SELECT DATE(`timestamp`) AS hora,
                       ...
                       MIN(indice_uv) AS indice_uv_min,
                       MAX(indice_uv) AS indice_uv_max,
                       AVG(radiacion_solar) AS radiacion_solar,
                       MIN(radiacion_solar) AS radiacion_solar_min,
                       MAX(radiacion_solar) AS radiacion_solar_max
                FROM meteo
            ...
            $query = "
                SELECT `timestamp` AS hora, indice_uv, radiacion_solar
                FROM meteo
                WHERE `timestamp` BETWEEN ? AND ?
