...
$date_format_php = 'H:i'; // Formato de fecha por defecto (H:i)

if ($tz_result && $tz_result->num_rows > 0) {
    $tz_row = $tz_result->fetch_assoc();
    // Limpieza: Eliminamos posibles barras invertidas ('Europe\/Madrid' -> 'Europe/Madrid')
...
if (isset($_GET['start']) && isset($_GET['end'])) {
    ...
    if ($start_dt && $end_dt) {
        ...
        if ($diff_days > 30) {
            ...
            $query = "
                SELECT DATE(`timestamp`) AS hora, AVG(humedad_interior) AS humedad_interior
                FROM meteo
            ...
            $query = "
                SELECT DATE_FORMAT(`timestamp`, '%Y-%m-%d %H:00:00') AS hora, AVG(humedad_interior) AS humedad_interior
                FROM meteo
                WHERE `timestamp` BETWEEN ? AND ?
            ...
            $query = "
                SELECT `timestamp` AS hora, humedad_interior
                FROM meteo
                WHERE `timestamp` BETWEEN ? AND ?
...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        ...
        // --- CONVERSIÓN DE HORA EN PHP (Solución 2) ---
        try {
            ...
            // En caso de error (ej. zona horaria no válida), mantenemos la hora UTC
        }
        // ----------------------------------------------

        $row["humedad_interior"] = (float) $row["humedad_interior"]; // forzamos tipo float
        $data[] = $row;
    }
}
