...
$PRESS_TREND_CACHE_KEY = 'clearsky_pressure_trend';
...
if (!$apcuAvailable) {
    ...
    $cache = apcu_fetch($PRESS_TREND_CACHE_KEY, $found);

    if ($found && isset($cache['computed_at']) && ($now - strtotime($cache['computed_at'])) < $PRESS_TREND_TTL) {
        $trendData = $cache;
    } else {
        $lockKey = $PRESS_TREND_CACHE_KEY . '_lock';
        if (apcu_add($lockKey, true, 10)) {
            $newTrend = computePressureTrend($mysqli);
            if ($newTrend !== null) {
                apcu_store($PRESS_TREND_CACHE_KEY, $newTrend);
                $trendData = $newTrend;
            } elseif ($found) {
